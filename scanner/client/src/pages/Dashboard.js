  ...
  return <div onClick={onClick} style={{padding:'9px 10px',borderRadius:7,marginBottom:3,cursor:'pointer',background:isSelected?'#1e2535':'transparent',border:`1px solid ${isSelected?c+'40':'transparent'}`}}
    ...
    <div style={{display:'flex',gap:7}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:5,marginTop:3}}><span className={`badge badge-${finding.severity}`} style={{fontSize:9}}>{finding.severity}</span><span style={{fontSize:10,color:'#4a5568'}}>CVSS {finding.cvss_score}</span></div>
      </div>
    </div>
  </div>;
}
  ...
  return <div style={{animation:'fadeInUp 0.2s ease'}}>
    <div style={{padding:18,background:'var(--bg-2)',borderRadius:11,border:`1px solid ${c}30`,marginBottom:14,borderLeft:`3px solid ${c}`}}>
      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:7}}>
        ...
        <span style={{fontSize:10,color:'#4a5568',fontFamily:'var(--font-mono)'}}>CVSS {finding.cvss_score} · {finding.type}</span>
      </div>
    ...
    <div style={{display:'flex',gap:4,marginBottom:14}}>
      {['overview','evidence','remediation'].map(t=>(
